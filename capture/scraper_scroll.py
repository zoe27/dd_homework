...
SCROLL_WAIT          = 1.2   # 滚动后等待渲染（秒）
CARD_OPEN_WAIT       = 1.8   # 点击卡片后等待面板渲染（秒）
CLOSE_WAIT           = 0.4   # 关闭面板后等待（秒）
SIMILARITY_THRESHOLD = 5.0   # 像素差阈值（越小越相似，低于此值判定为无变化）
SAME_COUNT_TO_STOP   = 2     # 连续相同次数判定到顶/底
MAX_SCROLL_STEPS     = 80    # 最大滚动次数

# 内容区比例（去掉顶部导航栏和底部输入栏）
CONTENT_TOP_RATIO    = 0.20  # 顶部跳过比例
CONTENT_BOTTOM_RATIO = 0.15  # 底部跳过比例
# 每次向上滚动 1/3 内容区高度，确保新旧截图差异足够大
SCROLL_UP_RATIO      = 1 / 3
...
def scroll(window: dict, delta: int) -> None:
    ...
    logger.info(f"滚动事件已发送 delta={delta}，消息区中心=({cx},{cy})")
...
def click(x: int, y: int) -> None:
    ...
    logger.info(f"点击事件已发送 ({x},{y})")


def strip_array(img: Image.Image, top: bool = False) -> np.ndarray:
    """裁剪中间内容区域（排除顶部导航栏和底部输入栏），转为 numpy 数组"""
    h, w = img.height, img.width
    top_skip = int(h * CONTENT_TOP_RATIO)
    bottom_skip = int(h * CONTENT_BOTTOM_RATIO)
    strip = img.crop((0, top_skip, w, h - bottom_skip))
    return np.array(strip, dtype=np.float32)


def content_scroll_delta(window: dict) -> int:
    """计算 1/3 内容区高度对应的滚动行数（正数，方向由调用方决定）"""
    content_h = window["height"] * (1 - CONTENT_TOP_RATIO - CONTENT_BOTTOM_RATIO)
    lines = max(8, int(content_h * SCROLL_UP_RATIO / 10))
    logger.info(f"内容区高度={content_h:.0f}px，1/3滚动={content_h*SCROLL_UP_RATIO:.0f}px，行数={lines}")
    return lines
    bottom = int(h * 0.7)
...
def is_same(img_a: Image.Image, img_b: Image.Image) -> bool:
    a = np.array(img_a.convert("L"))
    b = np.array(img_b.convert("L"))

    diff = np.mean(np.abs(a.astype(np.int16) - b.astype(np.int16)))

    logger.info(f"灰度差值: {diff:.2f}")

    return diff < SIMILARITY_THRESHOLD

# def is_same(img_a: Image.Image, img_b: Image.Image, top: bool = False) -> bool:
#     diff = np.abs(strip_middle(img_a) - strip_middle(img_b)).mean()
#     logger.info(f"内容区域像素差: {diff:.2f}（阈值 {SIMILARITY_THRESHOLD}）")
#     return diff < SIMILARITY_THRESHOLD


def scroll_and_wait(window: dict, delta: int) -> None:
    """滚动后等待页面稳定（串行：检测到页面变化后再返回）"""
    before = capture_window(window["window_id"])
    scroll(window, delta)
    # 最多等 2s，检测到页面变化即返回
    for _ in range(8):
        time.sleep(0.25)
        after = capture_window(window["window_id"])
        if before is not None and after is not None and not is_same(before, after):
            logger.info("页面已变化，继续")
            return
    logger.info("等待超时（页面可能未变化）")
...
def is_old_timestamp(text: str) -> bool:
    ...
    if text.strip() == "昨天" or text.strip().startswith("昨天 "):
        logger.info(f"发现'昨天'时间戳: {text!r}")
        return True
    ...
    for m in RE_TIMESTAMP_DATE.finditer(text):
        ...
        try:
            ...
            if msg_date < today:
                logger.info(f"发现早于今天的时间戳 {msg_date}: {text!r}")
                return True
        except ValueError:
            pass
    ...
    for t in all_texts:
        ...
        if dm:
            try:
                ...
                logger.info(f"截止时间日期: {card_date}  来源: {t!r}")
                break
            except ValueError:
                pass
        ...
        for r in ocr_results:
            ...
            if m:
                try:
                    ...
                    logger.info(f"标题日期: {card_date}  来源: {r.text!r}")
                    break
                except ValueError:
                    pass

    if card_date is None:
        logger.info("无法确定卡片日期，跳过")
        return None, None
...
def extract_panel_text(window: dict, debug: bool = False) -> str:
    ...
    logger.info(f"面板截图区域: x={panel_x}~{img.width}, 大小={panel_img.width}x{panel_img.height}")
    ...
    for r in results:
        if any(kw in r.text for kw in skip_keywords):
            logger.info(f"  跳过: {r.text!r}")
            continue
        logger.info(f"  保留: {r.text!r}")
        lines.append(r.text)
...
def scroll_to_bottom(window: dict, debug: bool = False) -> None:
    ...
    for step in range(MAX_SCROLL_STEPS):
        scroll_and_wait(window, SCROLL_DOWN_DELTA)
        img = capture_window(window["window_id"])
        if img is None:
        ...
        if prev_img is not None and is_same(prev_img, img):
            same_count += 1
            logger.info(f"底部相同计数: {same_count}/{SAME_COUNT_TO_STOP}")
...
def scan_upward(window: dict, debug: bool = False) -> list[RawMessage]:
    ...
    for step in range(MAX_SCROLL_STEPS):
        ...
        # 判断滚动后页面是否有变化（像素差小说明页面没动，可能已到顶）
        if prev_img is not None and is_same(prev_img, img):
            same_count += 1
            logger.info(f"内容相同计数: {same_count}/{SAME_COUNT_TO_STOP}（滚动后页面无变化）")
            if same_count >= SAME_COUNT_TO_STOP:
                logger.info(f"已到顶部，扫描结束（连续 {same_count} 次滚动后内容区域无变化）")
                break
        else:
            if prev_img is not None:
                diff = float(np.abs(strip_array(prev_img) - strip_array(img)).mean())
                logger.info(f"内容像素差: {diff:.2f}，页面有变化，继续扫描")
            same_count = 0
        prev_img = img
        ...
        if should_stop(results):
            logger.info("发现旧消息时间戳，停止扫描（今天之前的消息已出现）")
            break
        ...
        # 向上滚动（串行等待页面稳定，1/3内容区高度）
        scroll_and_wait(window, content_scroll_delta(window))
